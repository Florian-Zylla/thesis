...
for i in range(60):
    ...
    cv.imwrite("obstacle.png",frame)
    line = detected_edges[:,42]
    a = 42
    notFound = True
    while a < 84 and notFound:
        if line[a] == 255:
            depth[i] = pixel2dist(a)
            notFound = False
            surround[i,0] = depth[i] * math.sin(math.radians(i*6+45))
            surround[i,1] = depth[i] * math.cos(math.radians(i * 6+45))
            pixel_val[i] = a
    ...
    if notFound:
        print(i)
        time.sleep(2)
    #cv.imwrite("edge_output.png",detected_edges)
...
plt.figure(dpi=300)
#a[0,:]  = a[1,:]
plt.plot(surround[:,0], surround[:,1])
